 ...
 * 
 */
export class DeleteTodoLambda extends LambdaBase {
    ...
    async process(): Promise<APIGatewayProxyResult> {
        ...
        try {
            ...
            const res: APIGatewayProxyResult = {
                ...
                body: JSON.stringify({ message: "delete todo successful (or already deleted).", item: deleteResult })
            }
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    const lambda = new DeleteTodoLambda(event);
    return lambda.run();
};